...
const MorphMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      vNormal = normal;
      float n = fract(sin(dot(position + uTime, vec3(12.9898,78.233,37.719))) * 43758.5453);
      vec3 pos = position + normal * (n * 0.15);
      gl_Position = projectionMatrix * modelViewMatrix * vec4(pos, 1.0);
    }
  `,
  `
    void main() {
      gl_FragColor = vec4(0.0588,0.0902,0.1686,1.0);
    }
  `
)
extend({ MorphMaterial })

export default function MorphingFinger() {
  const ref = useRef()
  useFrame(({ clock }) => {
